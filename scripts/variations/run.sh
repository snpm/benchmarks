...
BENCH_INSTALL_COMMAND_SNPM="${BENCH_COMMAND_SNPM//\$\{HYPERFINE_ITERATION\}/run}"
...
BENCH_COMMAND_SNPM="snpm run test > $BENCH_OUTPUT_FOLDER/snpm-run-output-\${HYPERFINE_ITERATION}.log 2>&1"
...
hyperfine --ignore-failure \
  ...
  ${BENCH_INCLUDE_SNPM:+--prepare="$BENCH_INSTALL_COMMAND_SNPM"} \
  ${BENCH_INCLUDE_SNPM:+--command-name="snpm" "$BENCH_COMMAND_SNPM"} \
