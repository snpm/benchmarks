...
clean_snpm_cache() {
  echo "Cleaning snpm cache..."
  # Respect SNPM_HOME when set, otherwise cover the default paths.
  local snpm_home="${SNPM_HOME:-$HOME/.snpm}"
  local snpm_store="${SNPM_STORE_PATH:-$HOME/.snpm-store}"

  safe_remove "$snpm_home/cache"
  safe_remove "$snpm_home/data"
  safe_remove "$HOME/.cache/snpm/snpm"
  safe_remove "$HOME/.local/share/snpm/snpm"
  safe_remove "$snpm_store"
  safe_remove ".snpm-store"
}
...
clean_package_manager_files() {
  ...
  rm -rf .npm* .yarn* .pnp* .snpm* .snpmrc .snpm-store .snpm || true
}
